...
- (void)LlenarArrayTableHastaFinDeMes:(BOOL)aFinDeMes{
    ...
    if ([ArrayNotificaciones count] == 0) {
        [self MostrarMensajeHUD:@"No Measures" conEstado:0];
        return;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TestNextDetailViewController *nextDetailViewController = [[[TestNextDetailViewController alloc] initWithNibName:nil bundle:nil] autorelease];
...
- (void)viewDidLoad
{
    ...
    [self setTitle:@"Next"];
    [self setArrayNotificaciones:[NotificacionesDALC ListarNotificacionesPorTest:aTest]];
    [self LlenarArrayTableHastaFinDeMes:NO];
    [myTable reloadData];
}
