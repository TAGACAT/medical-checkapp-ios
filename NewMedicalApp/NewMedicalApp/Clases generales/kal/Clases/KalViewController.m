...
@synthesize dataSource, delegate, initialDate, selectedDate, NombreNotificacion, modo;
...
- (void)didSelectDate:(KalDate *)date
{
  ...
  [tableView flashScrollIndicators];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NombreNotificacion object:from];
}
...
- (void)loadView
{
  ...
  [self reloadData];
    ...
    self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStyleBordered target:self action:@selector(clicbtnBackCalendario:)] autorelease];
    
    if (modo == 1) {
        self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Never" style:UIBarButtonItemStyleBordered target:self action:@selector(clicbtnNeverCalendario:)] autorelease];
    }
...
-(void)clicbtnBackCalendario:(id)Sender{
    
    [self.navigationController popViewControllerAnimated:YES];
}


-(void)clicbtnNeverCalendario:(id)Sender{
    
    [[NSNotificationCenter defaultCenter] postNotificationName:NombreNotificacion object:nil];
    [self.navigationController popViewControllerAnimated:YES];
}
